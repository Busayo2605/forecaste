...
window.addEventListener("load", function (e) {
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function (position) {
      ...
      fetch(
        `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&appid=80871c3028e853529bd8461205cfb0a4&units=metric`
      )
        .then((res) => res.json())
        .then(function (data) {
          ...
          document.querySelector(
            ".min-temp"
          ).innerHTML = `${mintepvalue} &#176C`;
          document.querySelector(
            ".max-temp"
          ).innerHTML = `${maxtepvalue} &#176C`;
          document.querySelector(".date").innerHTML = date;
          document.querySelector(
            ".icon"
          ).innerHTML = `<img src="https://openweathermap.org/img/wn/${ic}@4x.png"/>`;
